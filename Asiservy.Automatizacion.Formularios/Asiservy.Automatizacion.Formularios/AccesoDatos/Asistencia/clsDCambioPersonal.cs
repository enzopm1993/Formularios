                    ...
                    if (tipo == "prestar")
                    {
                        ...
                        if (pListCambioPersonal.Count > 0)//si es un CAMBIO_PERSONAL(nuevo) que no está en la tabla CAMBIO_PERSONAL, hay que crearlo 
                        {
                            ...
                            foreach (var item in pListCambioPersonal.ToArray())
                            {
                                ...
                                {
                                    BuscarMovimientoPersonalDiario = db.MOVIMIENTO_PERSONAL_DIARIO.Where(x => x.Cedula == item.Cedula && x.FechaInicio == psfecha /*&& x.Asistencia.Value*/).OrderByDescending(x=>x.IdCambioPersonal).FirstOrDefault();//busco el registro en movimiento_personal_diario
