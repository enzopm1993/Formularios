            ...
            using (ASIS_PRODEntities entities = new ASIS_PRODEntities())
            {
                ...
                if (ControlHueso == null)
                {
                    if (doControl.TipoControlHueso == clsAtributos.Hueso || doControl.TipoControlHueso == clsAtributos.Roto)
                    {
                        ...
                        detalle = clsDAsistencia.ConsultaMovimientoPersonalDiario( doControl.HoraInicio, HoraIni, doControl.Linea).Where(x=> x.CodCargo==clsAtributos.CargoLimpiadora).ToList();
