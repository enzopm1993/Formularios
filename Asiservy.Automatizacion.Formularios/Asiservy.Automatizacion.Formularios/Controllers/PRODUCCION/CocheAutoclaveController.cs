...
namespace Asiservy.Automatizacion.Formularios.Controllers.PRODUCCION
{
    public class CocheAutoclaveController : Controller
    {
        ...
        #region REPORTE
        public ActionResult ReporteCocheAutoclave()
        {
            try
            {
                ViewBag.JavaScrip = "PRODUCCION/" + RouteData.Values["controller"] + "/" + RouteData.Values["action"];
        ...
        public ActionResult ReporteCocheAutoclaveDetallePartial(int OrdenFabricacion)
        {
            try
            {
                lsUsuario = User.Identity.Name.Split('_');
                ...
                // clsDEmpleado = new clsDEmpleado();
                // var Empleado = clsDEmpleado.ConsultaEmpleado(lsUsuario[1]).FirstOrDefault();
                var model = clsDCcocheAutoclave.ConsultaReporteCocheAutoclaveDetalle(OrdenFabricacion);
                if (!model.Any())
                {
                    return Json("0", JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult ConsultaOrdenesFabricacion(DateTime Fecha)
        {
            try
            {
                lsUsuario = User.Identity.Name.Split('_');
                ...
                clsDCcocheAutoclave = new clsDCcocheAutoclave();
                var Of=clsDCcocheAutoclave.ConsultarOrdenesFabricacion(Fecha);
                return Json(Of, JsonRequestBehavior.AllowGet);
            }
            catch (DbEntityValidationException e)
            {
                Response.StatusCode = (int)HttpStatusCode.InternalServerError;
