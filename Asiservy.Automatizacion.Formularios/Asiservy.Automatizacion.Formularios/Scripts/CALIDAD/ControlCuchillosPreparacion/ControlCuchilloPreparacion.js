    ...
    $.ajax({
        ...
        success: function (resultado) {
            ...
            if (resultado == "0") {
                ...
                //datosCabecera = resultado;
                //ConsultarControlCuchilloDetalle(0, resultado.IdControlCuchillo, 0, 1);
            }
            $('#cargac').hide();
        },
        error: function (resultado) {
...
function ValidarCuchilloExiste() {    
    ...
    if (datosDetalle.CodigoCuchillo == selected) {        
        ...
        var form_data = table.rows().data();
        for (var i in form_data) {
            if (form_data[i].IdCuchilloPreparacion == datosDetalle.IdCuchilloPreparacion) {
                return false;
            }
